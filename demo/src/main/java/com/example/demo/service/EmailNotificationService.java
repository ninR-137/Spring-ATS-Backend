...
public class EmailNotificationService {
    ...
    public boolean sendScheduledInterviewEmail(Applicant applicant, ApplicantInterview interview) {
        ...
        String subject = "Interview Scheduled: " + applicant.getName() + " - " + applicant.getRole();
        for (EmailRecipient recipient : recipients) {
            sendQuietly(
                recipient.getEmail(),
                subject,
                renderTemplate("interview-scheduled", buildInterviewScheduledModel(recipient.getName(), applicant, interview))
            );
        }

        sendQuietly(
            applicant.getEmail(),
            "Interview Scheduled for " + applicant.getRole() + " position",
            renderTemplate("interview-scheduled", buildInterviewScheduledModel(applicant.getName(), applicant, interview))
        );
        return true;
    }
    ...
    public boolean sendOrientationScheduledEmail(Applicant applicant, ApplicantOrientation orientation) {
        ...
        for (EmailRecipient recipient : recipients) {
            sendQuietly(
                recipient.getEmail(),
                subject,
                renderTemplate("orientation-scheduled", buildOrientationScheduledModel(recipient.getName(), applicant, orientation))
            );
        }
        ...
        sendQuietly(
            ...
            renderTemplate("orientation-scheduled", buildOrientationScheduledModel(applicant.getName(), applicant, orientation))
        );
        return true;
    }
